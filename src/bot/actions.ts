...
export const handleReplyAction = async (
  ...
  try {
    if (parentConversation) {
      ...
      await logger.saveLog("new_replay_faild", {});
    }
  } catch (error) {
...
export const handleBlockAction = async (
  ...
  try {
    if (parentConversation) {
      ...
      await logger.saveLog("user_block_faild", {});
    }
  } catch (error) {
  ...
  try {
    if (parentConversation) {
      ...
      if (blockList && blockList[parentConversation.from.toString()]) {
        ...
        await ctx.reply(HuhMessage);
        await logger.saveLog("user_unblock_faild1", {});
      }
    } else {
      await ctx.reply(HuhMessage);
      await logger.saveLog("user_unblock_faild2", {});
    }
  } catch (error) {
